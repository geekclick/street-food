"use client";
import React, { useState, useEffect } from "react";

const page = () => {
  const [alertMessage, setAlertMessage] = useState(false);
  const [input, setInput] = useState({
    type: "number",
  });
  const usePhone = () => {
    if (input.type === "number") {
      setInput({ type: "email" });
    } else {
      setInput({ type: "number" });
    }
  };
  const [inputValue, setInputValue] = useState("");

  const handleInputChange = (event) => {
    setInputValue(event.target.value);
  };

  const validateInput = () => {
    if (inputValue.length > 10) {
      setInputValue(inputValue.substring(0, 10));
    }
  };
  const handleSubmit = (event) => {
    if (inputValue.length == 10) {
      setAlertMessage(false);
    } else {
      event.preventDefault();
      setAlertMessage(true);
    }
  };

  useEffect(() => {
    validateInput();
  }, [inputValue]);
  return (
    <>
      <section className="bg-login_bg w-full h-screen bg-cover z-0">
        <div className="bg-login_img md:bg-center lg:bg-cover w-1/2 h-screen bg-no-repeat bg-blend-multiply z-10 hidden lg:block"></div>
        <div className="bg-black opacity-80 h-screen w-full z-20 flex flex-1 absolute top-0">
          <div className="lg:w-1/2 w-full h-screen flex justify-center items-center absolute right-0 z-30">
            <div className="bg-white lg:w-96 lg:h-96 w-full h-full rounded-none lg:rounded-3xl">
              <h1 className="text-center lg:text-2xl text-xl font-bold text-white bg-hirwa p-10 px-4 lg:static fixed top-0 w-full lg:rounded-t-3xl rounded-none">
                Verify you account
              </h1>
              <form action="forgot-password/verify">
                <div className="flex flex-col max-w-xs m-auto justify-center px-4 lg:px-0 items-center opacity-100 lg:mt-auto mt-28">
                  {input.type === "number" && (
                    <input
                      type="number"
                      name="phone-number"
                      id="phone-number"
                      placeholder="Enter your registered phone number"
                      maxLength={10}
                      value={inputValue}
                      onChange={handleInputChange}
                      className="lg:text-md text-sm w-full font-bold p-4 outline-none border-b-2 mt-10 mb-2 text-black bg-white"
                      autoFocus
                      required
                    />
                  )}
                  {alertMessage && (
                    <label
                      htmlFor="phone-number"
                      className="text-red-400 text-xs"
                    >
                      Please enter valid number
                    </label>
                  )}
                  {input.type === "email" && (
                    <input
                      type="emai;"
                      name="email"
                      id="email"
                      placeholder="Enter your registered email"
                      className="lg:text-md text-sm font-bold p-4 w-full outline-none border-b-2 mb-2 mt-10 text-black bg-white"
                      autoFocus
                      required
                    />
                  )}
                  <button
                    onClick={handleSubmit}
                    type="submit"
                    className="bg-hirwa p-3 rounded-3xl lg:w-3/4 mt-10 w-full hover:text-black hover:border border-green-500 text-white hover:bg-white font-bold lg:text-xl text-sm transition-all"
                  >
                    Submit
                  </button>
                  <p className="lg:text-sm text-xs text-center mt-4 mb-10 dark:text-gray-400">
                    <button
                      onClick={usePhone}
                      rel="noopener noreferrer"
                      className="focus:underline hover:underline mx-1 font-bold"
                    >
                      Use {input.type === "number" ? "Email " : "Phone number "}
                      instead
                    </button>
                  </p>
                </div>
              </form>
            </div>
          </div>
        </div>
      </section>
    </>
  );
};

export default page;
